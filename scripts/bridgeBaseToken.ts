...
const DEPOSIT_WAIT_SECONDS = parseInt(process.env.DEPOSIT_WAIT_SECONDS || '60');
...
async function main() {
  ...
  log('开始 L1 ↔ L2 Base Token 跨链测试');
  log('========================================');
  log(`L1 RPC: ${L1_RPC}`);
  log(`L2 RPC: ${L2_RPC}`);
  log(`Token Address: ${TOKEN_ADDRESS}`);
  log(`存款金额: ${DEPOSIT_AMOUNT} tokens`);
  log(`提现金额: ${WITHDRAW_AMOUNT} tokens`);
  log('========================================\n');
  ...
  const l1Erc20Contract = new ethers.Contract(TOKEN_ADDRESS, l1Erc20ABI, l1Provider);
  const l1BalanceBefore = await l1Erc20Contract.balanceOf(wallet.address);
  const l2BalanceBefore = await wallet.getBalance();
  log(`L1 Base Token Balance: ${ethers.formatEther(l1BalanceBefore)}`);
  log(`L2 Base Token Balance: ${ethers.formatEther(l2BalanceBefore)}`);

  // === 第二步：存款 L1 -> L2 ===
  log(`\n[2/5] 存款 ${DEPOSIT_AMOUNT} tokens 从 L1 到 L2...`);
  ...
  console.log('finalized', waitedTime, WITHDRAW_FINALIZE_WAIT, typeof waitedTime, typeof WITHDRAW_FINALIZE_WAIT, finalized);
  ...
  log('\n========================================');
  log('最终余额:');
  const l1BalanceAfter = await l1Erc20Contract.balanceOf(wallet.address);
  const l2BalanceAfter = await wallet.getBalance();
  log(`L1 Base Token Balance: ${ethers.formatEther(l1BalanceAfter)}`);
  log(`L2 Base Token Balance: ${ethers.formatEther(l2BalanceAfter)}`);
